...
Mbp::Mbp(ObsModelMode obsmodel_mode_, const Details &details, const Data &data, const Model &model, const AreaTree &areatree, const ObservationModel &obsmodel, const Output &output, Mpi &mpi) : state1(details,data,model,obsmodel), state2(details,data,model,obsmodel), comp(model.comp), lev(areatree.lev), trans(model.trans), details(details), data(data), model(model), areatree(areatree), obsmodel(obsmodel), output(output), mpi(mpi)
{
...
unsigned int Mbp::mcmc_updates(vector <Particle> &part, const vector <vector <double> > &param_samp, double EFcut_, double invT_,ParamUpdate pup_, ParamProp &paramprop)
{
	EFcut = EFcut_;
	invT = invT_;
	pup = pup_;
...
/// Updates a vector of particles using MBP proposals
unsigned int Mbp::mc3_mcmc_updates(Particle &part, const vector <vector <double> > &param_samp, double invT_, ParamUpdate pup_, ParamProp &paramprop)
{
	EFcut = UNSET;
	invT = invT_;
	pup = pup_;
...
void Mbp::update_particle(Particle &pa, const vector <Proposal> &prop_list, ParamProp &paramprop)
{
			...
			case MVN_PROP:
				{
					...
					switch(mvn.type){
						case OBS_PARAM: param_obsmodel_prop(mvn); break;
	...
	pa = initial->create_particle();                    // Places final state back into particle
}
...
{	
	...
	Status stat = mbp_pop_model(inf_update);
...
/// This performs a pure MBPs or a combination of MBP and simulation
Status Mbp::mbp_pop_model(InfUpdate inf_update)
{
	int num; MPI_Comm_rank(MPI_COMM_WORLD,&num); 
	
	auto ninf_total = 0u;                                  // Keeps track of the total number of infected individuals
	...
	for(auto sett = 0u; sett < details.ndivision; sett++){
		...
		for(auto c = 0u; c < data.narea; c++){               // Performs simulation / MBPs on the transitions
			...
			for(auto tr = 0u; tr < model.trans.size(); tr++){
				...
				auto type = model.trans[tr].type;
				
				for(auto dp = 0u; dp < data.ndemocatpos; dp++){	 
					pop_p = prop_pop[from][dp];
					num_i = init_tnum[tr][dp];
					if(pop_p <= 0) num_p = 0;
					else{
						switch(sorm){
							case SIMU:	
								num_p = poisson_sample(prop_tmean[tr][dp]);								
								break;
					...
					if(type == INFECTION_DIST){
						ninf_total += num_p;
						if(ninf_total >= model.maximum_infected){ timer[TIME_TRANSNUM].stop(); return FAIL;}
					}						
				}
			}
		}
...
double Mbp::get_al()
{
	...
	if(std::isnan(al)) emsgEC("Mbp",45);
				
	if(false) cout << al << " " << propose->EF << " " << initial->EF << " " <<  propose->Pr << " " << initial->Pr 
	               <<  "mbp al" << endl;
...
void Mbp::initialise_variables()
{
	...
	dImap.resize(model.ninfection_trans); dIdiag.resize(model.ninfection_trans);
...
void Mbp::mbp_fixedtree(FixedTree& ft)  
{
	...
	auto frac = ft.mbp_frac;                                              // Sets to simulate for specific areas
	for(auto c = 0u; c < data.narea; c++){             
		auto val = areatree.lev[ft.l].node[ft.n].mbp_sim[c];
		if(val == SIMU){
...
void Mbp::mbp_slicetime(SliceTime& st)  
{
	...
	auto frac = st.mbp_frac;                                          // Sets to simulate for a given time period
	for(auto sett = 0u; sett < details.ndivision; sett++){
...
/// Does a parameter proposal on the observation model
void Mbp::param_obsmodel_prop(MVN &mvn)
{
	initial->obsmodel_prop(mvn,EFcut,invT,obsmodel_mode);
}
