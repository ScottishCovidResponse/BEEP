...
mpirun -n 20  ./beepmbp inputfile="examples/infregion.toml" nchain=20 nsamp=100
...
Combine trace plots:
 ./beepmbp mode="combinetrace" dirs="a" output="para.txt" dist="dist.txt"
*/
  ...
  MPI_Comm_rank(MPI_COMM_WORLD,&num); core =num;
	...
	if (cmdlineparams.count("mode") == 1) {
		if(cmdlineparams["mode"] == "combinetrace"){
			...
			if(cmdlineparams.count("burnin") == 1) burnin = cmdlineparams["burnin"];
