...
/// Outputs the probabilit distributions generated by 
void Output::abcsmc_output(string file, const Generation &gen) const 
{
  ...
  distout << "# Posterior probability distributions for model parameters." << endl;
	...
	for(auto th = 0u; th < model.param.size()-1; th++){	
		auto N = gen.particle.size();
		vector <double> sumst;
		sumst.resize(N);       // Generate particle sampler
		double sum = 0; for(auto i = 0U; i < N; i++){ sum += gen.particle[i].w; sumst[i] = sum;}
		...
		for(auto s = 0u; s < smax; s++){
			...
			vec.push_back(gen.particle[k].paramval[th]);
		}
