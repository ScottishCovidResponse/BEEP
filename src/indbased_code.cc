...
In details.cc Details::Details(Inputs &inputs)
	switch(modeltype){
...
In Model.cc Model::Model
	if(modeltype == IND_MODEL){
		if((details.mode == ABC_SMC || details.mode == ABC_MBP) && maximum_infected == LARGE){
			emsgroot("Input file must contain a limit on the maximum number of individuals through 'maximum_infected'.");
		}
	}		
	
In void Model::add_transition
...
In void Inputs::find_transitions

			if(dist == "Lognorm"){
				distval = LOGNORM_DIST;
				emsgroot("Lognorm not working yet");
			}
			
			if(dist == "Gamma"){
				distval = GAMMA_DIST;
				emsgroot("Gamma not working yet");
			}
			
In string Model::print() 
	case GAMMA_DIST:
