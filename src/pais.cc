...
void PAIS::bootstrap(Generation &gen, vector<Particle> &part, vector <unsigned int> &partcopy, const double invT)
{
	...
	if(mpi.core == 0){
		...
		DinvT = quench_factor/sdav;                              // Sets the increase in inverse temperature
		if(invT_final != UNSET){
...
void PAIS::param_GR_clear()
{
	nproposal = 0; loop = 0; psamp_GR.clear();
}


/// Calculates when to terminate 
bool PAIS::terminate()
{
	auto term = false;
	loop++;
