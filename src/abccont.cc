/// Implements a version of ABC which uses continuum equations
...
/// Initilaises the ABCCONT class
ABCCONT::ABCCONT(const Details &details, const Data &data, const Model &model, Inputs &inputs, const Output &output, const ObservationModel &obsmodel, Mpi &mpi) : state(details,data,model,obsmodel), paramprop(details,data,model,output,mpi), details(details), data(data), model(model), output(output), obsmodel(obsmodel),mpi(mpi)
{	
...
unsigned int ABCCONT::mcmc_updates(const vector <ParamSample> &param_samp, double EFcut, ParamUpdate pup, double cor_max)
{
...
bool ABCCONT::terminate_generation(unsigned int g, double EFcut) const 
{
...
void ABCCONT::store_sample(Generation &gen)
{
...
bool PartEF_ord5 (PartEF p1,PartEF p2)                      
{ return (p1.EF < p2.EF); };  


/// Sets EF cut-off and works out which particles should be copied and which culled
void ABCCONT::EF_cutoff(Generation &gen, vector <Particle> &part, vector <unsigned int> &partcopy)
{	
	auto N = part.size();
	...
	if(mpi.core == 0){
		...
		sort(partef.begin(),partef.end(),PartEF_ord5);           // Sorts EFs
		...
		if(true){   // Copies and particles
			...
			for(auto ru = 0u; ru < nrun; ru++){	
				for(auto j = 0u; j < npart; j++){
					auto i = ru*npart + j;
					if(EFtot[i] >= EFcut){ 
						if(list[ru].size() == 0) emsgEC("ABCCONT",10);
						
						partcopy[i] = list[ru][list[ru].size()-1]; 
						list[ru].pop_back();
					}
				}
				if(list[ru].size() != 0) emsgEC("ABCCONT",11);
			}
			*/
		}
		else{
...
void ABCCONT::model_evidence(vector <Generation> &generation)
{
...
void ABCCONT::print_generation(const Generation &gen, const unsigned int g) const
{
