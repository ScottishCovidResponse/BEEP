...
struct FixedTree {        // This stores information about proposals that simulate a given set of areas
	unsigned int l;         // The level on the tree 
	unsigned int n;         // The node on that level
	double mbp_frac;        // The fraction of simulation (vs MBP)
...
{
public:
	ParamProp(const Details &details, const Data &data, const Model &model, const AreaTree &areatree, const Output &output, Mpi &mpi);
	...
	void zero_ntr_nac();
	vector <Proposal> get_proposal_list(const vector <vector <double> > &param_samp);
	...
	string print_proposal_information(bool brief) const;
	
private:
	void update(double &val, double acrate);
	void update_high(double &val, double acrate);
	void add_mvn(string name, ParamType type, double size);
	void add_single();
