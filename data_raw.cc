...
void Data::deaths_hospital_england()
{
	...
	for(auto col = 1u; col < tab.ncol; col++){
		auto t = details.gettime(tab.heading[col]) - details.start;
		
		IGageout << tab.heading[col] << "\t" << t;
		auto sum = 0u;
		for(auto row = 0u; row < tab.nrow; row++){
			IGageout << "\t" << tab.ele[row][col];
			sum += atoi( tab.ele[row][col].c_str());
		}
		IGageout << "\t" << sum << "\n";
	}
}
...
void Data::case_age_distribution()
{
	...
	for(auto row = 0u; row < tab.nrow; row++){
		if(atoi(tab.ele[row][1].c_str()) < 217){
			cout << tab.ele[row][0] << "\n";
			for(auto a = 0u; a < nage; a++){
				num[a] += atoi(tab.ele[row][2+a].c_str());
			}
		}
	}
...
{	
	...
	for(auto row = 0u; row < tabHD.nrow; row++){
		if(atoi(tabHD.ele[row][1].c_str()) < 217){
			cout << tabHD.ele[row][0] << "\n";
			for(auto a = 0u; a < nage; a++){
				num[a] += atoi(tabHD.ele[row][2+a].c_str());
				num[a] += atoi(tabCD.ele[row][2+a].c_str());
			}
		}
	}
	for(auto a = 0u; a < nage; a++) cout << a << " " << num[a] << " age dist\n";
	...
	for(auto row = 0u; row < admi.nrow-6; row += 7){
		...
		for(auto col = 2u; col < admi.ncol; col++){
			...
			for(auto day = 0u; day < 7; day++){
				sum += atoi(cases.ele[rowst+row+day][col].c_str()) - atoi(admi.ele[row+day][col].c_str());
			}
			if(sum < 0) sum = 0;
			IGageout << "\t" << sum;
		}
...
void Data::fractional_change(string file)
{
	...
	for(auto row = 0u; row < tab.nrow; row++){
		...
		for(auto col = 0; col < 4; col++){
			valst[col] = sum;
			sum += atoi(tab.ele[row][col+2].c_str());
		}
...
void Data::cases_age()
{
	...
	for(auto row = 0u; row < tab.nrow; row++){
		if(tab.ele[row][typecol] == "overview"){
			auto t = details.gettime(tab.ele[row][datecol] ) - details.start;
			
			IGageout << tab.ele[row][datecol] << "\t" << t;
			auto sumtot = 0.0;	
			sum = 0; 
			for(auto col = 0u; col < tab.ncol; col++){
				if(tab.heading[col] == "newCasesBySpecimenDate-0_4") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-5_9") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-10_14") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-15_19") sum += atoi(tab.ele[row][col].c_str());
			}
			IGageout << "\t" << sum;
			sum0to19 += sum;
			sumtot += sum;
			...
			sum = 0; 
			for(auto col = 0u; col < tab.ncol; col++){
				if(tab.heading[col] == "newCasesBySpecimenDate-20_24") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-25_29") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-30_34") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-35_39") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-40_44") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-45_49") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-50_54") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-55_59") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-60_64") sum += atoi(tab.ele[row][col].c_str());
			}
			IGageout << "\t" << sum;
			sum20to64 += sum;
			sumtot += sum;
			...
			sum = 0; 
			for(auto col = 0u; col < tab.ncol; col++){
				if(tab.heading[col] == "newCasesBySpecimenDate-65_69") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-70_74") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-75_79") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-80_84") sum += atoi(tab.ele[row][col].c_str());
			}
			IGageout << "\t" << sum;
			sum65to84 += sum;
			sumtot += sum;
			...
			sum = 0; 
			for(auto col = 0u; col < tab.ncol; col++){
				if(tab.heading[col] == "newCasesBySpecimenDate-85_89") sum += atoi(tab.ele[row][col].c_str());
				if(tab.heading[col] == "newCasesBySpecimenDate-90+") sum += atoi(tab.ele[row][col].c_str());
			}
			IGageout << "\t" << sum;
			sum85 += sum;
			sumtot += sum;
...
void Data::deaths_age()
{
	...
	for(auto col = 1u; col < tab.ncol; col++){	
		...
		sum = 0; for(auto row = 0u; row < 5; row++) sum += atoi(remove_comma(tab.ele[row][col]).c_str());
		IGageout << "\t" << sum;
		
		sum = 0; for(auto row = 5u; row < 14; row++) sum += atoi(remove_comma(tab.ele[row][col]).c_str());
		IGageout << "\t" << sum;
		
		sum = 0; for(auto row = 14u; row < 18; row++) sum += atoi(remove_comma(tab.ele[row][col]).c_str());
		IGageout << "\t" << sum;
		
		sum = 0; for(auto row = 18u; row < 20; row++) sum += atoi(remove_comma(tab.ele[row][col]).c_str());
		IGageout << "\t" << sum;
		
		auto sumtot = 0u; for(auto row = 0u; row < 20; row++) sumtot += atoi(remove_comma(tab.ele[row][col]).c_str());
		IGageout << "\t" << sumtot;
...
void Data::split_deaths_age_data()
{
	...
	for(auto row = 0u; row < nrow; row++){
		IGageout << row << " " << atoi(tab.ele[row][6].c_str()) << " " <<  atoi(tab2.ele[row][6].c_str()) << " " << atoi(CDtab.ele[row][CDcol].c_str()) + atoi(HDtab.ele[row][HDcol].c_str()) << " " <<  atoi(CDtab.ele[row][CDcol].c_str()) << " " << atoi(HDtab.ele[row][HDcol].c_str()) << "\n";
	}
	return;
	*/
	...
	for(auto loop = 0u; loop < 2; loop++){
		...
		auto cum = 0.0;
		for(auto row = 0u; row < nrow; row++){
			auto numCD = atoi(CDtab.ele[row][CDcol].c_str());
			auto numHD = atoi(HDtab.ele[row][HDcol].c_str());
			...
			auto sum = 0.0;
			for(auto a = 0; a < 4; a++){
				auto val = (unsigned int)(0.5+fac*atoi(tab.ele[row][2+a].c_str()));
				//auto val = (double)(0.5+fac*atoi(tab.ele[row][2+a].c_str()));
				IGageout << "\t" << val;
				sum += val;
			}
			cum += sum;
...
void Data::generatehospdata()
{
	...
	auto t = details.gettime("2020-03-19") - details.start;
	...
	for(auto w = 0u; w < nweek; w++){
		...
		auto sumtot = 0.0;
		for(auto row = 0u; row < tab.nrow; row++){
			unsigned int sum = 0; for(auto col = 1+w*step; col < 1+w*step+step; col++) sum += atoi(tab.ele[row][col].c_str());
			data << "\t" << sum;
			sumtot += sum;
		}	
		data << "\n";
...
void Data::generatedeathdata_england_wales()
{
	...
	for(auto row = 0u; row < tab.nrow; row++){
		...
		if(tab.ele[row][7] == "covid-19" && tab.ele[row][12] == "Occurrences"){	
			...
			auto w = atoi(st.substr(5,st.length()-5).c_str())-1;
			
			auto code = tab.ele[row][3];
			
			auto num = atoi(tab.ele[row][0].c_str());
...
void Data::generatedeathdata_scotland()
{
	...
	for(auto row = 0u; row < tab.nrow; row++){
		...
		if(sex == "All" && age == "All" && cause == "COVID-19 related" && date.substr(0,4) == "w/c "){
			...
			//cout << date << " dat\n";
			//cout << details.start+tt << " " <<  details.gettime(date) << " yy\n";
			if(details.start+tt > details.gettime(date)) emsg("Problem");
			auto t = details.gettime(date) - (details.start+tt);
			if(t < tmin) tmin = t;
			if(t > tmax) tmax = t;
...
void Data::plotrawdata()
{
	...
	for(auto col = 9u; col < tab.ncol-4; col++){
		auto sum = 0.0; for(unsigned int c = col-3; c <= col+3; c++) sum += atof(tab.ele[row_walk][c].c_str());
		val_walk.push_back(sum);
		
		sum = 0.0; for(unsigned int  c = col-3; c <= col+3; c++) sum += atof(tab.ele[row_drive][c].c_str());
		val_driving.push_back(sum);
		
		sum = 0.0; for(unsigned int c = col-3; c <= col+3; c++) sum += atof(tab.ele[row_transit][c].c_str());
	...
	for(auto& tdata : transdata){                                // Loads transition data for inference
		...
		for(auto r = 0u; r < nregion; r++){
			...
			for(auto row = 0u; row < tab2.nrow; row++){
				sum += atof(tab2.ele[row][col2].c_str());
			}
			
			outp << atof(tab.ele[r][col].c_str()) << " " << sum << "\n";
		}
	}
	*/
}
