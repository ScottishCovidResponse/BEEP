...
void output_timers(string file, Mpi &mpi)
{
	...
	if(mpi.core == 0){
		...
		if(time_av[TIME_UPDATEPOP] > 0) dia << per(time_av[TIME_UPDATEPOP]/time_av[TIME_ALG]) << " Update Population" << endl;
		...
		if(time_av[TIME_TRANSMEAN] > 0) dia << per(time_av[TIME_TRANSMEAN]/time_av[TIME_ALG]) << " Calculating mean for transitions" << endl;
		...
		if(time_av[TIME_MVN] > 0){
			...
			if(time_av[TIME_SLICETIME] > 0) dia << per(time_av[TIME_SLICETIME]/time_av[TIME_ALG]) << " Splicetime" << endl;
		}
